...
def run():
    ...
    def scrape_house_committee(cx, output_code, house_code):
        ...
        address_info = re.search(r"""Mailing Address:\s*(.*\S)\s*Telephone:\s*(\(202\) .*\S)""",
        ...
        cx["address"] = re.sub(r"(.*\S)(Washington, DC \d+)\s*(-\d+)?",
        ...
        for i, party, nodename in ((1, 'majority', 'primary'), (2, 'minority', 'secondary')):
            ...
            for rank, node in enumerate(dom.cssselect("#%s_group li" % nodename)):
                ...
                if found_name != moc['name']['official_full']:
                    print(("Name mismatch: %s (in our file) vs %s (on the Clerk page)" % (
                    moc['name']['official_full'], node.cssselect('a')[0].text_content())).encode("utf8"))
    ...
    def scrape_senate():
        ...
        for id, name in re.findall(
                r'value="/general/committee_membership/committee_memberships_(....).htm">(.*?)</option>', body,
                re.I | re.S):
            ...
            dom = lxml.etree.fromstring(
                body3.encode("utf8"))  # must be bytes to parse if there is an encoding declaration inside the string
    ...
    def scrape_senate_member(output_list, membernode, majority_party, is_joint):
        ...
        entry["party"] = party
        entry["rank"] = len([e for e in output_list if
                             e["party"] == entry["party"]]) + 1  # how many have we seen so far in this party, +1
