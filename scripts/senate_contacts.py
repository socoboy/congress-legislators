...
from scripts.utils import download, load_data, save_data, parse_date
...
def run():
    ...
    dom = lxml.etree.parse(
        io.BytesIO(body.encode("utf8")))  # file has an <?xml declaration and so must be parsed as a bytes array
    for node in dom.xpath("member"):
        ...
        term["party"] = {"D": "Democrat", "R": "Republican", "I": "Independent", "ID": "Independent"}[
    ...
    for node in dom.getroot():
        ...
        if bioguide_id in bioguide:
            ...
            if member_full in by_name:
                ...
                print(
                    "Bioguide ID '%s' and synthesized official name '%s' not recognized." % (bioguide_id, member_full))
                exit(0)
